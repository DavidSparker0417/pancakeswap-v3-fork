...
export function handleMint(event: Mint): void {
  let transaction = Transaction.load(event.transaction.hash.toHex()) as Transaction;
  let mints = transaction.mints;
  let mint = MintEvent.load(mints[mints.length - 1]) as MintEvent;
...
export function handleSwap(event: Swap): void {
  ...
  let derivedToken0AmountETH = (token0.derivedETH as BigDecimal).times(amount0Total);
  let derivedToken1AmountETH = (token0.derivedETH as BigDecimal).times(amount1Total);
